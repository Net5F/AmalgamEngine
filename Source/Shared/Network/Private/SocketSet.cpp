...
: numSockets(0)
{
    ...
    if (set == nullptr) {
        LOG_ERROR("Error allocating socket set: {}", SDLNet_GetError());
    }
}
...
void SocketSet::addSocket(const TcpSocket& socket)
{
    ...
    if (numAdded < 1) {
        LOG_ERROR("Error while adding socket: {}", SDLNet_GetError());
    }
    else {
        numSockets += numAdded;
    }
...
int SocketSet::checkSockets(unsigned int timeoutMs)
{
    ...
    if (numReady == -1) {
        LOG_INFO("Error while checking sockets: {}", SDLNet_GetError());
