...
//        for (unsigned int i = 0; i < Server::MessageSorter::VALID_DIFFERENCE; ++i) {
//            messageSorter.startReceive(i);
//            messageSorter.endReceive();
//        }
//
//        // Check that we're on the correct tick and everything works.
//        REQUIRE(messageSorter.getCurrentTick() == (Server::MessageSorter::VALID_DIFFERENCE));
////        REQUIRE(!(messageSorter.isTickValid(Server::MessageSorter::VALID_DIFFERENCE - 1)));
////        REQUIRE((messageSorter.isTickValid(Server::MessageSorter::VALID_DIFFERENCE * 2 - 1)));
//
//        // Push a message.
//        messageSorter.push(Server::MessageSorter::VALID_DIFFERENCE,
//            std::make_unique<std::vector<Uint8>>(std::vector<Uint8>{1, 2, 3}));
//
//        // Try to receive.
//        std::queue<BinaryBufferPtr>& queue
//            = messageSorter.startReceive(Server::MessageSorter::VALID_DIFFERENCE);
//        REQUIRE(!(queue.empty()));
//
//        BinaryBufferPtr message = std::move(queue.front());
//        REQUIRE(*message == std::vector<Uint8>({1, 2, 3}));
...
//        // Advance the tick.
//        messageSorter.endReceive();
////        REQUIRE(!(messageSorter.isTickValid(Server::MessageSorter::VALID_DIFFERENCE)));
//    }
//
//    SECTION("Wrap a lot.")
//    {
//        // Advance until we wrap.
//        unsigned int tickNum = Server::MessageSorter::VALID_DIFFERENCE * 407;
...
////            tickNum + Server::MessageSorter::VALID_DIFFERENCE - 1)));
//
//        // Push a message.
