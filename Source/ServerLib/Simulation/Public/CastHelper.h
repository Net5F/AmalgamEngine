...
public:
     ...
     * Note: Only one callback can be set for each type.
     */
    void setOnSpellCompleted(SpellType spellType,
                             std::function<void(const CastInfo&)> callback);
    ...
    std::unordered_map<SpellType, std::function<void(const CastInfo&)>>
        onSpellCompletedMap;
...
public:
    struct CastItemInteractionParams {
        ...
        /** (Optional) The client that requested this cast. If casterEntity is a
            client entity, you must fill this in. Otherwise it won't be
           replicated properly. */
